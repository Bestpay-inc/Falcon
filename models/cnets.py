...
def _make_causal_mask(
    input_ids_shape: torch.Size, dtype: torch.dtype, device: torch.device, past_key_values_length: int = 0, k_mask: int=0
):
    """
    ...
    for i in range(math.ceil(tgt_len//k_mask)+1):
        mask[i*k_mask:i*k_mask+k_mask, :i*k_mask+k_mask] = 0
...
def _expand_mask(mask: torch.Tensor, dtype: torch.dtype, tgt_len: Optional[int] = None):
    ...
    #print('inverted_mask.masked_fill(inverted_mask.to(torch.bool), torch.finfo(dtype).min):', inverted_mask.masked_fill(inverted_mask.to(torch.bool), torch.finfo(dtype).min))
...
class Model(nn.Module):
    def __init__(self,config,tree_choice=None, load_emb=False,path=None,bias=True):
        super().__init__()
        ...
        #self.cnn=nn.Conv1d(2*config.hidden_size,config.hidden_size,1,bias=bias)
        self.lstm=nn.LSTM(2*config.hidden_size,config.hidden_size,bias=bias,dropout=0.1,num_layers=2)
    ...
    def init_tree(self):
        self.tree = self.tree_choice
        self.tree_buffer=generate_tree_buffers(self.tree,self.embed_tokens.weight.device, self.config.k_mask)
    ...
    def forward(
        ...
        hidden_states = self.lstm(torch.cat((inputs_embeds, hidden_states), dim=-1))
    ...
    #@torch.no_grad()
    #def repeat_hidden(self,hidden_state,repeat_num, k_mask=2):
    #    new_hidden=[]
    #    for id,i in enumerate(repeat_num):
    #        new_hidden.append(hidden_state[:,id:id+k_mask].repeat(1,i,1))
    #    return torch.cat(new_hidden,dim=1)
    '''
    @torch.no_grad()
    def repeat_hidden(self,hidden_state,hs_repeat_indice,tree_mask=None,k_mask=2):
        n = hs_repeat_indice.shape[0]
        if tree_mask is None:
            assert n%k_mask==0
            return hidden_state[:,0:k_mask,:].repeat(1,n//k_mask,1)
        else:
            tree_mask = tree_mask.squeeze(0).squeeze(0)
            W,H = tree_mask.shape
            
            new_hidden = []
            for i in range(0, n, k_mask):
                query_mask = tree_mask[i, :W]
                for j in range(W):
                    if query_mask[j]==1:
                        new_hidden.append(hidden_state[:, j:j+1, :])

            return torch.cat(new_hidden, dim=1)
    '''
    @torch.no_grad()
    def repeat_hidden(self,hidden_state,parent_info,init=True,k_mask=2):
        if init:
            n = len(parent_info)
            return hidden_state[:,0:k_mask,:].repeat(1,n,1)

        new_hidden = []
        for i in range(len(parent_info)):
            index = parent_info[i]
            new_hidden.append(torch.narrow(hidden_state, 1, index*k_mask, k_mask))
        return torch.cat(new_hidden, dim=1)
    ...
    @torch.no_grad()
    def topK_genrate(self, hidden_states, input_ids, head, logits_processor, use_cache=True, k_mask=2, accept_length=1):
        ...
        if use_cache:
            if hasattr(self, "stable_kv") and self.stable_kv is not None:
                ...
                if accept_length+1<k_mask:
                    cache = [[] for i in range(len(self.stable_kv))]
                    for idx,data in enumerate(self.stable_kv):
                        for tensor in data:
                            cache[idx].append(tensor[:,:,:-(k_mask-1),:])
                    out_hidden, past_key_values = self(hidden_states, input_ids=input_ids[:,kv_len-(k_mask-1):], past_key_values=cache,use_cache=True)
                else:
            ...
            last_hidden = out_hidden[0, -self.config.k_mask:]
            out_hidden = out_hidden[:, -self.config.k_mask:]
            ...
            for i in range(len(self.tree_buffer['tree_indices'])):
                ...
                select_index=topk_index[self.tree_buffer['token_repeat_indices'][i]]      
                input_ids=select_index[None,:]
                '''
                if (i+1) == k_mask:
                    #a = time.time()
                    print('out_hidden:', out_hidden)
                    hidden_states=self.repeat_hidden(out_hidden,hs_repeat_indice,k_mask=k_mask)
                    print('hidden_states:', hidden_states)
                    #print('w/o for', time.time()-a)
                else:
                    #a = time.time()
                    print('out_hidden', out_hidden)
                    hidden_states=self.repeat_hidden(out_hidden,hs_repeat_indice,tree_mask=self.tree_buffer["attn_mask"][i], k_mask=k_mask)
                    print('hidden_states:',hidden_states)
                #print('hidden_states:', hidden_states)
                '''
                if (i+1)==k_mask:
                    hidden_states=self.repeat_hidden(out_hidden,self.tree_buffer["total_parent_info"][i], k_mask=k_mask)
                else:
                    hidden_states=self.repeat_hidden(out_hidden,self.tree_buffer["total_parent_info"][i], init=False, k_mask=k_mask)
...
if __name__=="__main__":
    config = FConfig.from_pretrained('config.json')
    model = Model(config,load_emb=True,path="vicuna_v13/7B/")
